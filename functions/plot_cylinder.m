...
function [] = plot_cylinder(H,height,radius,color,alpha,n,linestyle)
    ...
    fill3(tb(1,:),tb(2,:),tb(3,:),'w','FaceColor',color,'FaceAlpha',alpha,'LineStyle',linestyle)
    fill3(tt(1,:),tt(2,:),tt(3,:),'w','FaceColor',color,'FaceAlpha',alpha,'LineStyle',linestyle)
    
    for k = 1:1:(n-1)
        fill3([tb(1,k) tb(1,k+1) tt(1,k+1) tt(1,k)],[tb(2,k) tb(2,k+1) tt(2,k+1) tt(2,k)],[tb(3,k) tb(3,k+1) tt(3,k+1) tt(3,k)],'w','FaceColor',color,'FaceAlpha',alpha,'LineStyle',linestyle)
    end
